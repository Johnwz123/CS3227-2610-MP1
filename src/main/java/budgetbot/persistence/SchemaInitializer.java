...
final class SchemaInitializer {
  ...
  static void initialize(Connection connection) throws SQLException {
    try (Statement statement = connection.createStatement()) {
      ...
      statement.execute(
          "CREATE TABLE IF NOT EXISTS settings (id INTEGER PRIMARY KEY CHECK (id = 1), rollover_enabled INTEGER NOT NULL, warning_threshold INTEGER NOT NULL)");
      statement.execute(
      ...
      statement.execute(
          "CREATE TABLE IF NOT EXISTS monthly_budgets (category_id INTEGER NOT NULL, month TEXT NOT NULL, base_amount TEXT NOT NULL, carryover TEXT NOT NULL, rollover_enabled INTEGER NOT NULL, warning_threshold INTEGER NOT NULL, PRIMARY KEY(category_id, month), FOREIGN KEY(category_id) REFERENCES categories(id) ON DELETE CASCADE)");
      statement.execute(
